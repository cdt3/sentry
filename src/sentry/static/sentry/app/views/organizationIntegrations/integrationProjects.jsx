...
class IntegrationProjects extends AsyncComponent {
  ...
  constructor(props, context) {
    super(props, context);
    this.state = {projects: [], adding: false};
  }
  ...
  renderDropdown() {
    ...
    const items = this.props.projects
      .map(p => p.slug)
      .filter(p => !this.state.integration.projects.includes(p))
      .map(p => ({searchKey: p, value: p, label: p}));
  ...
  renderBody() {
    const projects = this.state.integration.projects;
      ...
      <Panel>
        ...
        <PanelBody>
          ...
          {projects.map(project => (
            <PanelItem key={project} align="center">
              <Box flex="1">{project}</Box>
              <Box pr={1}>
                <Button size="xsmall">Configure</Button>
              </Box>
              <Box>
                <Confirm
                  ...
                  message={
                    <React.Fragment>
                      ...
                      <p>
                        {t(
                          'Removing the this integration from the project will' +
                            'clear any project specific configurations'
                        )}
                      </p>
                    </React.Fragment>
                  }
